<script setup>
  ...
  function changeState() {
    console.log('kkk');
    if (
      data.value.params?.[0]['state'] != 'working' ||
      data.value?.params[1]['state'] != 'working'
    ) {
      color.value = '#DB8181';
    }
    return color.value;
...
<template>
  <div
    class="h-[17vh] w-[13vw] rounded-[0.6vw] bg-gradient-to-b from-[#D6EBFF] to-[#272933] p-[0.1vw] opacity-40">
    <div class="flex h-full gap-[0.4vw] rounded-[0.6vw] bg-[#3E404C]">
      <div
        :style="{ 'border-color': changeState() }"
        class="relative ml-[0.83vw] mt-[0.83vw] h-[14.07vh] w-[3.33vw] rounded-lg border-2 border-solid">
        <div class="absolute left-[1.25vw] top-[1.1vh]">
          <!-- <img src="../assets/icons/crab.svg"/> -->
          <svg
            class="h-[1.1vh] w-[0.625vw]"
            viewBox="0 0 12 12"
            :fill="changeState()"
        ...
        <div class="absolute left-[0.95vw] bottom-[0.39vh]">
          ...
          <svg
            class="h-[15.2vh] w-[0.625vw]"
            viewBox="0 0 14 112"
            :stroke="changeFirstState()"
            fill="none"
            xmlns="http://www.w3.org/2000/svg">
            ...
            <path
              d="M4.75 12L5.875 12L7 12"
              stroke-width="2"
              stroke-miterlimit="10"
              stroke-linecap="round" />
            <path
              d="M4.75 34L5.875 34L7 34"
              stroke-width="2"
              stroke-miterlimit="10"
              stroke-linecap="round" />
            <path
              d="M4.75 56L5.875 56L7 56"
              stroke-width="2"
              stroke-miterlimit="10"
              stroke-linecap="round" />
            <path
              d="M4.75 78L5.875 78L7 78"
              stroke-width="2"
              stroke-miterlimit="10"
              stroke-linecap="round" />
            <path
              d="M4.75 100L5.875 100L7 100"
              stroke-width="2"
              stroke-miterlimit="10"
              stroke-linecap="round" />
        ...
        <div
          :style="{ 'background-color': changeFirstState(), height: firstParamHeight() }"
          class="absolute left-[1.07vw] bottom-[1.11vh] w-[0.35vw]"></div>
        <div
          :style="{ 'background-color': changeSecondState(), height: secondParamHeight() }"
          class="absolute right-[1.07vw] bottom-[1.11vh] w-[0.35vw]"></div>

        <div class="absolute top-[2.05vh] left-[0.04vw] flex flex-col gap-[0.75vh]">
          <div
            :style="{ color: changeFirstState() }"
            v-for="item in firstParamScale"
            class="flex h-[1.3vh] w-[0.78vw] justify-end font-euclid text-[0.52vw]">
            {{ item }}
          </div>
        </div>
        ...
        <div class="absolute top-[2.05vh] right-[0.04vw] flex flex-col gap-[0.75vh]">
          <div
            :style="{ color: changeSecondState() }"
            v-for="item in secondParamScale"
            class="flex h-[1.3vh] w-[0.78vw] justify-start font-euclid text-[0.52vw] text-[#009CF3]">
            {{ item }}
          </div>
        </div>
      </div>
      ...
      <div class="mt-[0.83vw] flex flex-col">
        <div
          class="rounded-tr-0 rounded-br-0 flex h-[2.9vh] w-[8.3vw] items-center rounded-tl-3xl rounded-bl-3xl bg-[#272933]">
        ...
        <div class="mt-[2vh] flex flex-col">
          <div class="flex flex-row gap-[0.4vw]">
            <div
              class="ml-[0.3vw] flex w-[4.01vw] items-center justify-end font-euclid text-[0.625vw] font-medium text-[#D6EBFF]">
              <div class="truncate">{{ data.params?.[0]['name'] }}</div>
              :
            </div>
            <div
              :style="{ color: changeFirstState() }"
              class="flex w-[2.81vw] items-center truncate font-euclid text-[0.83vw] font-medium">
              <div class="mr-0.5 truncate">{{ data.params?.[0]['value'] }}</div>
              {{ data.params?.[0]['type'] }}
            </div>
          </div>

          <div class="mt-[0.3vh] flex flex-row gap-[0.4vw]">
            <div
              class="ml-[0.3vw] flex w-[4.01vw] items-center justify-end overflow-hidden font-euclid text-[0.625vw] font-medium text-[#D6EBFF]">
              <div class="truncate">{{ data.params?.[1]['name'] }}</div>
              :
            </div>

            <div
              :style="{ color: changeSecondState() }"
              class="flex w-[3.5vw] items-center truncate font-euclid text-[0.83vw] font-medium">
              <div class="mr-0.5 truncate">{{ data.params?.[1]['value'] }}</div>
              {{ data.params?.[1]['type'] }}
            </div>
          </div>
        </div>

        <div class="mt-[3.4vh] flex flex-row items-center gap-[0.2vw]">
          <img src="../assets/icons/crab/vector.svg" class="ml-[1.2vw]" />
          <div
